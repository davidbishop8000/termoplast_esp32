#ifndef INC_MANUAL_PAGE_H_
#define INC_MANUAL_PAGE_H_

const char manual_html[] PROGMEM = R"rawliteral(
<!DOCTYPE HTML><html><head><title>Manual mode</title><meta charset="utf-8">
	<style>
		*{
	...
	<script>
		...
		setInterval(async () => {
			let x_coord = Joy3.GetX();
			let y_coord = Joy3.GetY();
			console.log(x_coord, " | ", y_coord);
			let comm = 0;
			if (y_coord > 50) comm = 1;
			else if (y_coord < -50) comm = 2;
			else if (x_coord > 50) comm = 3;
			else if (x_coord < -50) comm = 4;
			try {
				const response = await fetch(`manual?comm=${comm}`)				
				if (response.ok) {
					const result = await response.json();
					//console.log(`ok`);
				}
